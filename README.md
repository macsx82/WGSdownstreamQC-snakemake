...
In the folder **"HweMissingClean"** will be saved the files automatically filtered by HWE and missing rate. Those files will be used for subsequent steps.
The folder arrayAF will contain tables and plots regarding a comparison between the WGS data and the provided SNP array data. The files named as "WGS_ITA_PREREL_chr19_MERGED_ARRAY_af_extrDiff.txt" should be used for variants removal
