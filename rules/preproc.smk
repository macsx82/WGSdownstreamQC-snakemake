...
rule reapplyVQSRsnps:
	...
	shell:
		"""
		{params.bcftools_bin} view -r {params.chrom} -i 'VQSLOD >= {params.vqslod_thr}' -v snps {input} -O z -o {output[0]}
...
rule reapplyVQSRindels:
	...
	shell:
		"""
		{params.bcftools_bin} view -r {params.chrom} -i 'VQSLOD >= {params.vqslod_thr}' -v indels {input} -O z -o {output[0]}
		{params.bcftools_bin} index -t {output[0]}
		"""
